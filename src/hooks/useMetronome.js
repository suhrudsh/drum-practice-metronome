 ...
 */
export function useMetronome({ tempo, speedMultiplier, numerator, volume, onBeat }) {
  ...
  const scheduleNote = useCallback((beatIndex, time) => {
  ...
  const scheduler = useCallback(() => {
    ...
    while (nextNoteTimeRef.current < ctx.currentTime + SCHEDULE_AHEAD_SEC) {
      ...
      currentBeatRef.current = (currentBeatRef.current + 1) % numeratorRef.current
    }
