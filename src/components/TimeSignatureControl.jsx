...
export default function TimeSignatureControl({ numerator, denominator, onNumeratorChange, onDenominatorChange }) {
  return (
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={() => onNumeratorChange(Math.max(MIN_BEATS, numerator - 1))}
          >
            &minus;
          </button>
          <span className="font-mono text-xl text-brass-bright min-w-[18px] text-center">{numerator}</span>
          <button
            className={stepBtnClass}
            aria-label="More beats per measure"
            onClick={() => onNumeratorChange(Math.min(MAX_BEATS, numerator + 1))}
          >
            +
          </button>
        </div>
        ...
        <div className="flex gap-1.5 flex-wrap">
          {NOTE_VALUES.map((val) => (
            <button
              ...
              className={
                'font-mono text-[13px] rounded-lg border px-3 py-1.5 cursor-pointer ' +
                'focus-visible:outline focus-visible:outline-2 focus-visible:outline-brass-bright focus-visible:outline-offset-2 ' +
                (denominator === val
